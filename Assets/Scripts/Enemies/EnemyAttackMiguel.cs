...
public class EnemyAttackMiguel : MonoBehaviour
{
    [Header("=== DETECCIÓN ===")]
    ...
    public string paramIsHidding = "";               // Ataque subterráneo (dejar vacío si el Animator no lo tiene)
    ...
    // Verifica si el parámetro existe en el Animator antes de usarlo
    ...
    {
        if (anim != null && !string.IsNullOrEmpty(paramName) && HasParam(paramName))
    ...
    {
        if (anim == null) return;
        if (HasParam(paramHorizontal)) anim.SetFloat(paramHorizontal, dir.x);
        if (HasParam(paramVertical)) anim.SetFloat(paramVertical, dir.y);
        if (HasParam(paramSpeed)) anim.SetFloat(paramSpeed, dir.magnitude);
    }
